...
#define NUMMINES 64                     // maximum of mines at one time. will remove a random one if the limit is exceeded
#define LASERLENGTH 1500.0              // maximum length for laserbeam
...
public server_frame(){
  ...
  while((ent = find_ent_by_class(ent,"item_mine")) > 0){
    ...
    if(f[0] >= MINE_CHECKED){
      ...
      new target[3]
      new Float:ret[3]
      angle_vector_forward(angles,ret)
...
angle_vector_forward(Float:angle[3], Float:vector[3])
{
	new Float:ignored[3]
	engfunc(EngFunc_AngleVectors, angle, vector, ignored, ignored)
}
